...
Run this script after the jobs submitted by batch_shape are completed. You can check them with 'squeue'.

$0 [Shape output directory] [ groupfile ]
  Give the full pathname for each argument.
  1st arg: Shape output directory. Same as the 2nd argument used for the batch_shape.sh script.
  2nd arg: csv file with list of subjcts. Same as the 4th argumnent used for the batch_shape.sh script.
